...
function FindIdForm(props) {
  ...
  const submit = (e) => {
    ...
    if (istoggle == true) {
      if (findId.name == "") {
        ...
        axios
          .post("http://localhost:8080/findById", findId)
          .then((res) => {
            console.log(res);
            Swal.fire(res.data);
  ...
  const sendEmailSMS = (e) => {
    alert("sendEmailSMS");
    axios
      .post("http://localhost:8080/send-mail", null, {
        params: {
          email: findId.email
        }
      })
  ...
  return (
    <div className="findIdform">
      <form onSubmit={submit}>
        <div className="findIdformWrap">
          ...
          <div className="findwrap">
            <div className="findwrapTab" onClick={handleButtonClick1}>
              ...
              <p>
                본인확인용 이메일 주소와 입력한 이메일 주소가 동일 시 인증번호를
                받을 수 있습니다.
              </p>
        ...
        <a href="login" className="loginFormBtn">
          로그인
        </a>
      </form>
    </div>
